...
in `intelliDots`. Normal hosted-provider use does not require `localLLM`,
...
```bash
./install.sh --only toolsDots.aiTools.opencode
```

Or directly:

```bash
cd toolsDots/aiTools/opencode
./install.sh
```
...
- `nvim` automatically loads this project's stowed toggleterm keymaps
  ...
  Without `opencode` installed, `nvim`'s toggleterm plugin loads normally
  with no OpenCode-specific keymaps and no errors.

Neither `nvim` nor this integration is required for the core `opencode-online`/
...
```bash
cd toolsDots/aiTools/opencode   # from your intelliDots checkout
./install.sh
exec "$SHELL" -l
```
