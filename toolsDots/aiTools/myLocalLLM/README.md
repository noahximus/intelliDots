...
```bash
cd toolsDots/aiTools/myLocalLLM
...
- `myNvim` automatically loads the supplied CodeCompanion integration when it
  detects this project’s installed integration file.
- `myOpenCode` can use the stack through its optional `opencode-local` helper.
