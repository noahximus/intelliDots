...
`intelliDots`. It can be installed without `essentialDots`, `myLocalLLM`, or
...
```bash
./install.sh --only toolsDots.devTools.myNvim
```

Or directly:

```bash
cd toolsDots/devTools/myNvim
./install.sh
```
...
selection. The file is owned by `myLocalLLM`; neither project requires the
other.
...
`myOpenCode`; without it, toggleterm loads normally with no OpenCode keymaps
and no errors.
