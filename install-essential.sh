...
usage() {
  cat <<'EOF'
Usage: ./install-essential.sh [options]

Installs every node in features.yaml at the essential profile, with macOS
...
directly when you want to choose what gets installed, or
install-everything.sh for the same fixed choice at the full profile.
...
# Fixed choice: every node, essential profile, macOS defaults applied,
# pipx apps installed. Only operational flags (how, not what) may be added.
args=(--essential --all --macos-defaults --pipx)

while [[ $# -gt 0 ]]; do
  case "$1" in
    --no-bootstrap|--with-turbo-fieldfare|--continue-on-error|--stop-on-error|--dry-run) args+=("$1") ;;
    -h|--help) usage; exit 0 ;;
    # Reject anything that would turn this back into a per-node or
    # per-profile choice; install.sh already covers that use case.
    --essential|--full|--all|--macos-defaults|--pipx|--only|--only=*|--skip|--skip=*)
      echo "install-essential.sh always installs every node's essential profile; $1 is not accepted here." >&2
      exit 2
      ;;
